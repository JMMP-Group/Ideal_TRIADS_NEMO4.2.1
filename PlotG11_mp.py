#!/usr/bin/env python
#%run /Users/agn/VC_code/KNcode/PlotG11_mp.py
import numpy as np
...
class NEMOGM:
    def __init__(self,nstepmax=None,nstep=0,dn=1,skew=False,filetype='restart',dirtry=None,salinity=False,ix=1,eos='linearT',FK=False):
        import glob
    ...
    #             dirtry +='OLDISO_aeiv1000_ts1hr_aht0.1_explicit/'
        ...
        if 'e3t' in names:
            ...
            zz = fmesh.variables['gdepw'][0,:,1:-1,ix+1]
        ...
        if 'e3t' in names:
            ...
            self.zt = zt[:,:,ix]
            self.zz = zz
            self.zv = np.zeros([nzz,nyy])
            self.zv[:,1:-1] = .5*(zz[:,1:] + zz[:,:-1])
            self.zv[:,0] = zz[:,0]
            self.zv[:,-1] = zz[:,-1]
        else:
        ...
        if filetype == 'restart':
            ...
            Temperature = 'tn'
            Salinity = 'sn'
            filepath = pjoin(dirtry,'all_restarts.nc')
            filepathv = None
        elif filetype == 'average':
            frag = '*5d*grid_T.nc'
            fragv = '*5d*grid_V.nc'
            Temperature = 'votemper'
            Salinity = 'vosaline'
            filepath = glob.glob(pjoin(dirtry,frag))[0]
            filepathv = glob.glob(pjoin(dirtry,fragv))[0]
        elif filetype == 'instant':
            frag = '*5d*grid_Ti.nc'
            fragv = '*5d*grid_Vi.nc'
            Temperature = 'votemper'
            Salinity = 'vosaline'
        ...
        files = glob.glob(pjoin(dirtry,frag))
        if not os.access(filepath,os.R_OK):
            f= netCDF4.MFDataset(files)
            print 'Using multifiles:\n %s' % '\n'.join(files)
        else:
            f = netCDF4.Dataset(filepath)
            print 'Using file: %s' % filepath
            if filetype == 'restart':
                fv = f
            else:
                fv = netCDF4.Dataset(filepathv)

        TNd = f.variables[Temperature]
        SNd = f.variables[Salinity]
        nt = TNd.shape[0]
        print 'Tshape=',TNd.shape
        if psiname in fv.variables.keys():
            psiyNd =  fv.variables[psiname]
            self.psi = np.zeros([nzz,nyy],dtype=psiyNd.dtype)
        else:
            self.psi = None
        if nstepmax is None:
            nstepmax = nt
        ...
        if self.psi is None:
            ...
            hi_levels = np.linspace(-slimit,slimit,11)
            hi_anns = np.linspace(-slimit,slimit,6)

            self.psiLevels = np.union1d(low_levels,hi_levels)
            self.psiAnn = np.union1d(low_anns,hi_anns)


        if filetype == 'restart':
            self.time = f.variables['time_counter'][:]
            self.time *= f.variables['rdttra1'][:]
        elif filetype == 'average':
            time_secs = f.variables['time_centered'][:]
            self.time = (time_secs + .5*time_secs[1] - 1.5*time_secs[0])#/86400.
        elif filetype == 'instant':
            time_secs = f.variables['time_instant'][:]
            self.time = (time_secs + time_secs[1] - 2.*time_secs[0])#/86400.
        ...
        # wants theta0,T0,depth0 (m)
        neos_nemo = {'JM94':0,'linearT':1,'linearTS':2}
        nemo_rho.eos.set_eos(neos_nemo[eos])
        self.depth_km = 0.5
        # nemo_rho.eos.initialize_psigma_0(10.,35.,500.)
        ...
        self.filepathv = filepathv
        self.Temperature = Temperature
        self.Salinity = Salinity
        self.psiname = psiname
        # self.f =f
        # self.fv =fv
        # print 1/0
    ...
    def find_sigma(self,T,S):
        # rho = nemo_rho.eos.psigma_0(T.ravel(),S.ravel()).reshape(T.shape)
        rho = nemo_rho.eos.sigma_n(T.ravel(),S.ravel(),self.depth_km).reshape(T.shape)
        return rho
    ...
    def output(self):
        ...
        if nnc < 0:
            ...
            f = netCDF4.Dataset(self.filepath)

            if self.psi is not None:
                if self.filetype == 'restart':
                    fv = f
                else:
                    fv = netCDF4.Dataset(self.filepathv)

                psi = fv.variables[self.psiname][nnc,:,:-1,self.ix]
                if self.filetype != 'restart':
                    fv.close()
            else:
                psi = self.psi

            TNd,SNd = [f.variables[x] for x in (self.Temperature,self.Salinity)]
            S,T = SNd[nnc,:-1,1:-1,1:-1], TNd[nnc,:-1,1:-1,1:-1]
            f.close()
            print 'In output Tmax=',T.max(),'Smax=',S.max()
...
if  __name__=='__main__':
    ...
    parser.add_option('--filetype',
              help='whether uses restarts, average or instant files; default is average',
              default='average')
    parser.add_option('--FK',dest='FK',action='store_true',
              help='plot out mle psi ; default is False',
              default=False)
    parser.add_option('--ix',dest='ix',type='int',
              help='(python) i index for NS section relative to physical domain',
              default=1)
    parser.add_option('-e','--euler',dest='euler',action='store_true',
              help='mix every dtime; default is False',
              default=False)
    parser.add_option('--eos', dest='eos',help='equation of state: choices are linearT,linearTS,JM94', default='linearT')
    parser.add_option('--processes', dest='processes',help='no of processes', type='int', default=None)
    ...
    if nemo:
        GM = NEMOGM(nstepmax=None,nstep=0,dn=1,skew=True,dirtry=currdir,salinity=salinity,eos=options.eos,FK=options.FK,ix=options.ix,filetype=options.filetype)
    else:
    ...
    if nemo:
        # vmin = 25.0
        # vmax = 28.
        vmin = ma.masked_where(GM.rhmsk,GM.rho0).min()
        vmax = ma.masked_where(GM.rhmsk,GM.rho0).max()
    else:
        vmin = -0.25
        vmax = 0.075
        # vmin = -1.0
    ...
    if nemo:
        nsteps = range(nstep,nstepmax+1)
        if options.processes>1:
            pool = Pool(processes=options.processes)
            failures = pool.map(dopict,nsteps)
            pool.close()
        else:
            for n in nsteps:
                dopict(n)
